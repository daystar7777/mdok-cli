...
if (mode === "slow-save"||mode==='safety-slow-save') {
...
process.chdir(dir);
// Isolated app discovery: never depend on or launch the user's installed app.
process.env.MDOK_DESKTOP_APP=join(dir,'missing.app');
if(mode==='desktop'){
  const app=join(dir,'mdok.app');await fs.mkdir(join(app,'Contents/MacOS'),{recursive:true});
  await fs.writeFile(join(app,'Contents/MacOS/mdok-desktop'),'fixture',{mode:0o755});
  process.env.MDOK_DESKTOP_APP=app;
}
process.stdout.write('AUDIT_DIRECTORY='+dir+'\n');
...
const b = mode === 'same-preview' ? a : mode === 'compare-long' ? Array.from({length:100},(_,i)=>`new line ${i}`).join('\n') : "BRAVO document";
...
if(mode.startsWith('safety'))await fs.writeFile(join(dir,'.mdok.json'),JSON.stringify({lang:process.env.MDOK_AUDIT_LANG||'en',gitSync:false,externalChanges:mode==='safety-auto'?'auto':mode==='safety-keep'?'keep':'ask',autoSave:['safety-autosave','safety-slow-save','safety-recovery'].includes(mode)}));
try {
  const {runTui} = await import("../dist/tui.js");
  const initialTabs = ["two","slow-save","same-preview"].includes(mode) ? [{path:pathA,content:a},{path:pathB,content:b}] : [{path:pathA,content:a}];
  if(mode==='safety-recovery')initialTabs[0]={path:pathA,content:'RECOVERED DRAFT',baseline:a,diskToken:null};
  if(mode==='safety-recovery-unreadable'){
    initialTabs[0]={path:pathA,content:'RECOVERED DRAFT',baseline:'',diskToken:null};
    await fs.writeFile(pathA,Buffer.from([0,255]));
  }
  if(mode==='safety-aliases'){
    const alias=join(dir,'alias.md'),hard=join(dir,'hard.md');await fs.symlink(pathA,alias);await fs.link(pathA,hard);
    initialTabs.push({path:alias,content:a},{path:hard,content:a});
  }
  ...
  process.stdout.write("\nAUDIT_RESULT="+JSON.stringify({a:await fs.readFile(pathA,"utf8").catch(()=>null),b:await fs.readFile(pathB,"utf8"),session,saved,language,html:await fs.readFile(join(dir,"a.html"),"utf8").catch(()=>null)})+"\n");
